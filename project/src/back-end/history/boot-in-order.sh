...
echo "Wait (indefenitly) until the DB creation (name: ${DB_NAME_H})."
echo "The DB URL is: ${DB_URL_H}"
until curl --request PUT ${DB_URL_H} ; do
  echo -e "\t DB (${DB_NAME_H}) wasn't created - trying again later..."
  sleep 2
done

echo "DB (${DB_NAME_H}) was created!"
